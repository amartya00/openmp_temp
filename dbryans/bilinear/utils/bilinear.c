...
void bilinear (unsigned int *in, unsigned short width, unsigned short height, unsigned short theta, unsigned int *out) {
        ...
        for (j = 0; j < width -1; j++){
            ...
            if (si > 0 && si < height -1){
                if(sj >0 && sj < width -1){
                    ...
                    w1 = (ir >> 8) & (0xFF);
                    w2 = (ir >> 8) & (0xFF);
                    out[i * width + j] = (0xFF - w1)*((0xFF-w2)*in[si * width + sj] + w2*in[(si+1) * width + sj]) + w1*((0xFF-w2)*in[si * width + sj +1] + w2*in[(si+1) * width + sj +1]);
