...
/* Copyright (C) 2011
   ...
   - Declaring an array with variable length not supported by TI compiler 
   - alloc/free changed to gomp_alloc/gomp_free */
	   ...
	   unsigned flags __attribute__((unused)))
{
...
#ifndef __GNUCS__
  char buf[200]; //TODO find a good value here
#endif
  ...
  if (!if_clause || team == NULL
	...
	task.in_tied_task = thr->task->in_tied_task;
      thr->task = &task;
      gomp_flush0(); /* TSP - point imm. following generation of explicit task*/
...
gomp_barrier_handle_tasks (gomp_barrier_state_t state)
{
  ...
  while (1)
	...
	{
	  thr->task = child_task;
          gomp_flush0(); /* flush before calling child task */
	  ...
	  if (to_free)
	    {
          ...
          gomp_flush0(); /* flush before returning to parent */
	  return;
	}
