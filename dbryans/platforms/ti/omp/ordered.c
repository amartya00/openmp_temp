...
gomp_ordered_first (void)
{
  ...
  gomp_sem_post (team->ordered_release[thr->ts.team_id]);		/*using QMSS semaphores*/
}
...
gomp_ordered_last (void)
{
  ...
  if (--ws->ordered_num_used > 0)
	...
	next = 0;
      ...
      gomp_sem_post (team->ordered_release[next_id]);		/*using QMSS semaphores*/
    }
}
...
gomp_ordered_static_init (void)
{
  ...
  gomp_sem_post (team->ordered_release[0]);		/*using QMSS semaphores*/
}
...
gomp_ordered_static_next (void)
{
  ...
  gomp_sem_post (team->ordered_release[id]);		/*using QMSS semaphores*/
}
...
gomp_ordered_sync (void)
{
  ...
  /* OMP v3.1, 2.8.6, p81, l16 - "At entry to ordered region" */
  gomp_flush0();
}
...
{
  /* OMP v3.1, 2.8.6, p81 l16 - "At exit from ordered region" */
  gomp_flush0();
}
