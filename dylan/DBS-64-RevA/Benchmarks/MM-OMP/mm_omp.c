...
void omp_multiply_dmatrix(dmatrix A, dmatrix B, dmatrix C)
/*
...
void omp_multiply_lmatrix(lmatrix A, lmatrix B, lmatrix C)
/*
   Multiplies A and B, and puts the result in C, uses OpenMP.
   C should be zeroed and initialized with size: (A->rows, B->cols).
*/
...
int main (int argc, char *argv[])
{
  ...
  dmatrix B = new_random_dmatrix(rows, cols, 0, 10);
  dmatrix C = new_dmatrix(rows, cols);
  ...
  lmatrix Y = new_random_lmatrix(rows, cols, 0, 10);
  lmatrix Z = new_lmatrix(rows, cols);
